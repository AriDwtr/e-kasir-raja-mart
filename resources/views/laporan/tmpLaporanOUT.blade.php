...
<body>
    ...
    <h3>Laporan Transaksi Out / Keluar</h3>
        ...
        <tbody>
            ...
            @foreach ($data as $index => $nilai)
                <tr>
                    ...
                    <td>{{ $nilai->nm_user }}</td>
                    <td>{{ $nilai->jml_brg }}</td>
                    <td>{{ formatRupiah($nilai->hrg_brg_jual / $nilai->jml_brg) }}</td>
                    <td>{{ formatRupiah($nilai->hrg_brg_jual) }}</td>
                </tr>
                @php
                $total += ($nilai->hrg_brg_jual); // Menambahkan jumlah subtotal pada total
                @endphp
            @endforeach
            <tr class="total-row">
